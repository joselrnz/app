...
export async function generateStaticParams() {
  const slugs = getAllSlugs('circuits')
  return slugs.map((slug) => ({ slug }))
}

export default async function CircuitArticlePage({ params }: PageProps) {
  const article = await getMarkdownBySlug('circuits', params.slug)

  if (!article) {
    notFound()
  }

  const relatedArticles = getRelatedArticles('circuits', params.slug, 3)
  const folderStructure = getFolderStructure('circuits')
  const { previous, next } = await getAdjacentArticles('circuits', params.slug)
    ...
    <div className="min-h-screen bg-black text-white">
        ...
        <div className="flex max-w-[1920px] mx-auto">
          ...
          <div className="flex-1 px-6 lg:pl-8">
            ...
            <div className="flex items-center gap-2 text-sm text-gray-400 mb-8">
              ...
              <span>→</span>
              <Link href="/tech/circuits" className="hover:text-white transition-colors">Circuits</Link>
              <span>→</span>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="border-t border-white/10 mt-16 pt-8">
                ...
                <div className="flex gap-4">
                  <a
                    href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(article.metadata.title)}&url=${encodeURIComponent(`https://joselrnz.com/tech/circuits/${params.slug}`)}`}
                    target="_blank"
                    rel="noopener noreferrer"
                  ...
                  <a
                    href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(`https://joselrnz.com/tech/circuits/${params.slug}`)}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-lg transition-colors"
                  >
      ...
      <section className="py-24 bg-gradient-to-b from-gray-950 to-black border-t border-white/10">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <h2 className="text-4xl md:text-5xl font-bold mb-6">Explore more projects</h2>
          <p className="text-xl text-gray-300 mb-12">
            Check out other circuit and electronics projects.
          </p>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            >
              <span className="relative z-10 font-medium">All Circuits</span>
            </Link>
