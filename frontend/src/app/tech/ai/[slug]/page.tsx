...
export async function generateStaticParams() {
  const slugs = getAllSlugs('ai')
  return slugs.map((slug) => ({ slug }))
}

export default async function AIArticlePage({ params }: PageProps) {
  const article = await getMarkdownBySlug('ai', params.slug)
  if (!article) notFound()
  const relatedArticles = getRelatedArticles('ai', params.slug, 3)
  const folderStructure = getFolderStructure('ai')
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="pt-16"><TechNavigation currentCategory="ai" /></div>
      ...
      <article className="relative bg-gradient-to-b from-black via-gray-950 to-black pt-32 pb-24">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex items-center gap-2 text-sm text-gray-400 mb-8">
            ...
            <span>→</span>
            <Link href="/tech/ai" className="hover:text-white transition-colors">AI</Link>
            <span>→</span>
          ...
          <div className="flex gap-12 items-start">
            ...
            <SidebarNavigation category="ai" currentSlug={params.slug} folderStructure={folderStructure} />
            ...
            <div className="flex-1 min-w-0 border-t border-white/10 pt-12">
              ...
              <div className="border-t border-white/10 mt-16 pt-8">
                ...
                <div className="flex gap-4">
                  <a
                    href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(article.metadata.title)}&url=${encodeURIComponent(`https://joselrnz.com/tech/ai/${params.slug}`)}`}
                    target="_blank"
                  ...
                  <a
                    href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(`https://joselrnz.com/tech/ai/${params.slug}`)}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-lg transition-colors"
                  >
          ...
          <div className="max-w-7xl mx-auto px-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {relatedArticles.map((related, index) => (
                <Link key={index} href={`/tech/ai/${related.slug}`} className="button-border-light group p-6 rounded-2xl border border-white/10 bg-white/5 hover:bg-white/10 transition-all duration-300">
      ...
      <section className="py-24 bg-gradient-to-b from-gray-950 to-black border-t border-white/10">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <h2 className="text-4xl md:text-5xl font-bold mb-6">Explore more AI projects</h2>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link href="/tech/ai" className="button-border-light group relative flex items-center gap-2 px-8 py-4 rounded-full border transition-all duration-300 bg-white text-black border-white hover:bg-white/90">
              <span className="relative z-10 font-medium">All AI Projects</span>
            </Link>
