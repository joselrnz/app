...
import { getMarkdownBySlug, getAllSlugs, getRelatedArticles, getFolderStructure } from '@/lib/markdown'
...
export async function generateStaticParams() {
  const slugs = getAllSlugs('linux')
  return slugs.map((slug) => ({ slug }))
}

export default async function LinuxArticlePage({ params }: PageProps) {
  const article = await getMarkdownBySlug('linux', params.slug)

  if (!article) {
    notFound()
  }

  const relatedArticles = getRelatedArticles('linux', params.slug, 3)
  const folderStructure = getFolderStructure('linux')
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="pt-16">
        <TechNavigation currentCategory="linux" />
      </div>
      ...
      <article className="relative bg-gradient-to-b from-black via-gray-950 to-black pt-32 pb-24">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex items-center gap-2 text-sm text-gray-400 mb-8">
            ...
            <span>→</span>
            <Link href="/tech/linux" className="hover:text-white transition-colors">Linux</Link>
            <span>→</span>
          ...
          <header className="mb-12 max-w-4xl">
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm text-gray-400 mb-6">
              ...
              {article.metadata.author && (<><span>•</span><span>By {article.metadata.author}</span></>)}
            </div>
            <p className="text-xl text-gray-300 mb-6">{article.metadata.description}</p>
            <div className="flex flex-wrap gap-2">
              {article.metadata.tags.map((tag, index) => (
                <span key={index} className="px-3 py-1 bg-emerald-500/20 text-emerald-400 rounded-full text-sm">{tag}</span>
              ))}
            </div>
          </header>

          <div className="flex gap-12 items-start">
            {/* Sidebar Navigation - Sticky on the left */}
            <SidebarNavigation category="linux" currentSlug={params.slug} folderStructure={folderStructure} />

            {/* Main Content */}
            <div className="flex-1 min-w-0 border-t border-white/10 pt-12">
              ...
              <div className="border-t border-white/10 mt-16 pt-8">
                ...
                <div className="flex gap-4">
                  <a
                    href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(article.metadata.title)}&url=${encodeURIComponent(`https://joselrnz.com/tech/linux/${params.slug}`)}`}
                    target="_blank"
                  ...
                  <a
                    href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(`https://joselrnz.com/tech/linux/${params.slug}`)}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-lg transition-colors"
                  >
          ...
          <div className="max-w-7xl mx-auto px-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {relatedArticles.map((related, index) => (
                <Link key={index} href={`/tech/linux/${related.slug}`} className="button-border-light group p-6 rounded-2xl border border-white/10 bg-white/5 hover:bg-white/10 transition-all duration-300">
                  <h3 className="text-lg font-semibold mb-2 group-hover:text-emerald-400 transition-colors">{related.metadata.title}</h3>
                  <p className="text-sm text-gray-400 mb-4 line-clamp-2">{related.metadata.description}</p>
      ...
      <section className="py-24 bg-gradient-to-b from-gray-950 to-black border-t border-white/10">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <h2 className="text-4xl md:text-5xl font-bold mb-6">Explore more guides</h2>
          <p className="text-xl text-gray-300 mb-12">Check out other Linux and IoT guides.</p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link href="/tech/linux" className="button-border-light group relative flex items-center gap-2 px-8 py-4 rounded-full border transition-all duration-300 bg-white text-black border-white hover:bg-white/90">
              <span className="relative z-10 font-medium">All Linux Guides</span>
            </Link>
